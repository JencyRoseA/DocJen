pipeline {
    agent any 
    environment {
        DOCKERHUB_CREDENTIALS = credentials('sample1')
    }
    stages {
        ...
        stage('login to dockerhub') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'sample1', usernameVariable: 'DOCKERHUB_USERNAME', passwordVariable: 'DOCKERHUB_PASSWORD')]) {
